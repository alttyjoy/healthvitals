...
export default function SharedReportView() {
  ...
  if (error && !needsPassword) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-8 max-w-md text-center">
        <Warning weight="duotone" className="w-12 h-12 text-[#EF4444] mx-auto mb-4" />
        <h2 className="text-xl font-medium text-[#0F172A] mb-2" style={{ fontFamily: 'Outfit' }}>Report Unavailable</h2>
  ...
  if (needsPassword) return (
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] px-4">
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-8 max-w-md w-full" data-testid="shared-report-password">
        <div className="text-center mb-6">
          <Lock weight="duotone" className="w-12 h-12 text-[#0EA5E9] mx-auto mb-3" />
          <h2 className="text-xl font-medium text-[#0F172A]" style={{ fontFamily: 'Outfit' }}>Password Protected</h2>
          <p className="text-sm text-[#64748B] mt-1">This report requires a password to view</p>
        </div>
        ...
        <Input type="password" value={password} onChange={e => setPassword(e.target.value)}
          ...
          className="rounded-xl border-[#E2E8F0] bg-[#F8FAFC] mb-4" />
        <Button onClick={unlockReport} className="w-full rounded-full bg-[#0EA5E9] hover:bg-[#0284C7] text-white" data-testid="shared-report-unlock-btn">
          Unlock Report
        </Button>
      </div>
    </div>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            <Heartbeat weight="duotone" className="w-7 h-7 text-[#0EA5E9]" />
            <span className="text-lg font-semibold text-[#0F172A]" style={{ fontFamily: 'Outfit' }}>VitalTrack</span>
          </div>
          <h1 className="text-2xl font-medium text-[#0F172A]" style={{ fontFamily: 'Outfit' }}>
            Health Report for {report.user_name}
          </h1>
          <p className="text-sm text-[#64748B] mt-1">{report.start_date} to {report.end_date}</p>
        </div>
        ...
        {Object.entries(vitalGroups).map(([vk, entries], idx) => {
          ...
          return (
            <div key={vk} className="bg-white border border-[#E2E8F0] rounded-2xl p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="ml-auto text-sm text-[#64748B]">Avg: <strong className="text-[#0F172A]">{avg}</strong></span>
              </div>
              <ResponsiveContainer width="100%" height={200}>
                {vital.chartType === 'bar' ? (
                  <BarChart data={data}><CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                    <XAxis dataKey="date" fontSize={10} tick={{ fill: '#64748B' }} /><YAxis fontSize={10} tick={{ fill: '#64748B' }} />
                  ...
                  <LineChart data={data}><CartesianGrid strokeDasharray="3 3" stroke="#E2E8F0" />
                    <XAxis dataKey="date" fontSize={10} tick={{ fill: '#64748B' }} /><YAxis fontSize={10} tick={{ fill: '#64748B' }} />
                    <Tooltip /><Legend />
