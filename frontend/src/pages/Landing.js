  ...
  return (
    <div className="min-h-screen bg-[#FAFAF9]">
      ...
      <nav className="fixed top-0 w-full z-50 glass-nav border-b border-[#EAE7E1]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex items-center justify-between h-14 sm:h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {user ? (
              <Link to="/dashboard">
                <Button className="rounded-full bg-[#2D4A3E] hover:bg-[#1E332A] text-white px-4 sm:px-6 text-sm" data-testid="go-to-dashboard-btn">
              ...
              <>
                <Link to="/login" className="hidden sm:block">
                  <Button variant="ghost" className="text-[#6E6E6A] hover:text-[#2C2C2A] text-sm" data-testid="nav-login-btn">Sign In</Button>
                </Link>
                <Link to="/register">
                  <Button className="rounded-full bg-[#2D4A3E] hover:bg-[#1E332A] text-white px-4 sm:px-6 text-sm" data-testid="nav-register-btn">
                    Get Started
                  </Button>
                </Link>
              </>
            ...
            <button
              className="md:hidden p-1.5 rounded-lg hover:bg-[#EAE7E1] transition-colors"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              data-testid="mobile-nav-toggle"
            >
              {mobileMenuOpen ? <X className="w-5 h-5 text-[#2C2C2A]" /> : <List className="w-5 h-5 text-[#2C2C2A]" />}
            </button>
          </div>
        </div>
        {/* Mobile menu */}
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-[#EAE7E1] bg-white/95 backdrop-blur-md animate-fade-in">
            <div className="px-4 py-3 space-y-1">
              <a href="#features" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-2.5 text-sm text-[#6E6E6A] hover:text-[#2C2C2A] hover:bg-[#FAFAF9] rounded-lg">Features</a>
              <a href="#pricing" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-2.5 text-sm text-[#6E6E6A] hover:text-[#2C2C2A] hover:bg-[#FAFAF9] rounded-lg">Pricing</a>
              <a href="#faq" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-2.5 text-sm text-[#6E6E6A] hover:text-[#2C2C2A] hover:bg-[#FAFAF9] rounded-lg">FAQ</a>
              {!user && (
                <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="block px-3 py-2.5 text-sm text-[#2D4A3E] font-medium hover:bg-[#FAFAF9] rounded-lg">Sign In</Link>
              )}
            </div>
          </div>
        )}
      ...
      <section className="pt-24 sm:pt-32 pb-16 sm:pb-24 px-4 sm:px-6 md:px-12 lg:px-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative">
          <div className="max-w-3xl animate-fade-in-up">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl tracking-tight font-light text-[#2C2C2A] mb-4 sm:mb-6 leading-[1.1]" style={{ fontFamily: 'Outfit' }}>
              Track Your Health,<br />
              <span className="font-medium text-[#2D4A3E]">Every Single Day</span>
            </h1>
            <p className="text-base sm:text-lg text-[#6E6E6A] leading-relaxed mb-8 sm:mb-10 max-w-xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <Link to="/register">
                <Button className="rounded-full bg-[#2D4A3E] hover:bg-[#1E332A] text-white px-6 sm:px-8 py-3 sm:py-3.5 text-sm sm:text-base shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 w-full sm:w-auto" data-testid="hero-cta-btn">
                  Start Tracking Free <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </Link>
              <a href="#features">
                <Button variant="outline" className="rounded-full border-[#EAE7E1] text-[#6E6E6A] px-6 sm:px-8 py-3 sm:py-3.5 text-sm sm:text-base hover:bg-[#EAE7E1] w-full sm:w-auto" data-testid="hero-learn-more-btn">
                  Learn More
                </Button>
              </a>
            </div>
          ...
          <div className="mt-10 sm:mt-16 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 animate-fade-in-up stagger-2">
            {[{ n: '12', l: 'Health Vitals' }, { n: '3', l: 'Flexible Plans' }, { n: '24/7', l: 'Access Anywhere' }, { n: '100%', l: 'Data Privacy' }].map((s, i) => (
              <div key={i} className="bg-white border border-[#EAE7E1] rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
                <div className="text-2xl sm:text-3xl font-semibold text-[#2D4A3E] mb-1" style={{ fontFamily: 'Outfit' }}>{s.n}</div>
                <div className="text-xs sm:text-sm text-[#6E6E6A]">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" className="py-16 sm:py-24 px-4 sm:px-6 md:px-12 lg:px-24 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            {features.map((f, i) => (
              <div key={i} className={`bg-[#FAFAF9] border border-[#EAE7E1] rounded-xl sm:rounded-2xl p-6 sm:p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-fade-in-up stagger-${i % 3 + 1}`}>
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl bg-[#2D4A3E]/10 flex items-center justify-center mb-4 sm:mb-5">
                  <f.icon weight="duotone" className="w-5 h-5 sm:w-6 sm:h-6 text-[#2D4A3E]" />
                </div>
                <h3 className="text-lg font-medium text-[#2C2C2A] mb-2" style={{ fontFamily: 'Outfit' }}>{f.title}</h3>
      ...
      <section id="pricing" className="py-16 sm:py-24 px-4 sm:px-6 md:px-12 lg:px-24">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10 sm:mb-16">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl tracking-tight font-medium text-[#2C2C2A] mt-3" style={{ fontFamily: 'Outfit' }}>
      ...
      <section className="py-16 sm:py-24 px-4 sm:px-6 md:px-12 lg:px-24">
        <div className="max-w-4xl mx-auto bg-[#2D4A3E] rounded-2xl sm:rounded-3xl p-8 sm:p-12 md:p-16 text-center">
      ...
      {/* Footer */}
      <footer className="py-8 sm:py-12 px-4 sm:px-6 md:px-12 lg:px-24 border-t border-[#EAE7E1]">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-4 sm:gap-6 md:flex-row md:justify-between">
          ...
          <div className="flex flex-wrap justify-center gap-4 sm:gap-6 text-sm text-[#6E6E6A]">
            <a href="#features" className="hover:text-[#2C2C2A] transition-colors">Features</a>
            <a href="#pricing" className="hover:text-[#2C2C2A] transition-colors">Pricing</a>
            <a href="#faq" className="hover:text-[#2C2C2A] transition-colors">FAQ</a>
