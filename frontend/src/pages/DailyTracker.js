...
export default function DailyTracker() {
  ...
  if (loading) return (
    <div className="space-y-4 animate-pulse">
      <div className="h-10 bg-[#EAE7E1] rounded-xl w-48" />
      <div className="h-[400px] bg-[#EAE7E1] rounded-2xl" />
    </div>
  );
  ...
  return (
    <div className="max-w-full space-y-6 animate-fade-in-up" data-testid="daily-tracker">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-medium text-[#2C2C2A]" style={{ fontFamily: 'Outfit' }}>Daily Tracker</h1>
          <p className="text-sm text-[#6E6E6A]">Enter your health vitals for each day</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 bg-white border border-[#EAE7E1] rounded-xl px-1 py-1">
            ...
            <span className="text-xs text-[#6E6E6A] px-2 min-w-[120px] text-center">
          ...
          <Button onClick={handleSaveAll} disabled={saving} className="rounded-full bg-[#2D4A3E] hover:bg-[#1E332A] text-white px-5" data-testid="tracker-save-all-btn">
      ...
      {enabledVitals.length === 0 ? (
        <div className="bg-white border border-[#EAE7E1] rounded-2xl p-12 text-center">
          <Note weight="duotone" className="w-12 h-12 text-[#6E6E6A] mx-auto mb-4" />
          <h3 className="text-lg font-medium text-[#2C2C2A] mb-2" style={{ fontFamily: 'Outfit' }}>No Vitals Enabled</h3>
          <p className="text-sm text-[#6E6E6A] mb-4">Go to Settings to enable the vitals you want to track.</p>
        </div>
      ) : (
        <div className="bg-white border border-[#EAE7E1] rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.02)]">
          <div className="overflow-x-auto">
            <table className="tracker-table w-full text-sm" data-testid="tracker-table">
              <thead>
                <tr>
                  <th className="sticky left-0 z-20 bg-[#FAFAF9] px-6 py-4 text-left text-xs font-semibold text-[#2C2C2A] tracking-wide uppercase border-b border-r border-[#EAE7E1] min-w-[180px]">
                    Vital
                  </th>
                  {dates.map(date => {
                    ...
                    return (
                      <th key={date} className={`px-4 py-3 text-center border-b border-[#EAE7E1] min-w-[100px] ${isToday ? 'bg-[#2D4A3E]/5' : 'bg-[#FAFAF9]'}`}>
                        <div className="text-xs text-[#6E6E6A]">{f.day}</div>
                        <div className={`text-base font-medium ${isToday ? 'text-[#2D4A3E]' : 'text-[#2C2C2A]'}`}>{f.date}</div>
                        <div className="text-xs text-[#6E6E6A]">{f.month}</div>
                      </th>
                    );
                  })}
                </tr>
              ...
              <tbody>
                {enabledVitals.map(vk => {
                  ...
                  return (
                    <tr key={vk} className="hover:bg-[#FAFAF9]/50 transition-colors">
                      <td className="sticky left-0 z-10 bg-white px-6 py-3 border-b border-r border-[#EAE7E1]">
                        <div className="flex items-center gap-2">
                          <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: vital.color }} />
                          <div>
                            <div className="font-medium text-[#2C2C2A] text-sm">{vital.name}</div>
                            <div className="text-xs text-[#6E6E6A]">{vital.unit}</div>
                          </div>
                        </div>
                      </td>
                      {dates.map(date => {
                        ...
                        const isToday = date === today;
                        return (
                          <td key={date} className={`px-2 py-2 border-b border-[#EAE7E1] text-center ${isToday ? 'bg-[#2D4A3E]/[0.02]' : ''}`}>
                            {isDualValue ? (
                              <div className="flex items-center gap-1 justify-center">
                                ...
                                />
                                <span className="text-[#6E6E6A] text-xs">/</span>
                                <input
          ...
          {/* Legend */}
          <div className="px-6 py-3 border-t border-[#EAE7E1] flex flex-wrap items-center gap-4 text-xs text-[#6E6E6A]">
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#588157]" /> Normal</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#E9C46A]" /> Warning</span>
            <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-[#D96C4E]" /> Critical</span>
