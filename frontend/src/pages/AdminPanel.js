...
import {
  Users, ChartLine, CreditCard, FileArrowDown, MagnifyingGlass, ShieldCheck, Envelope, Article, Trash, PencilSimple, Plus, Bell, Clock, Tag
...
export default function AdminPanel() {
  ...
  const openAddCoupon = () => {
    ...
    setCouponForm({ code: '', discount_percent: 10, max_uses: 0, valid_plans: [], expires_at: '', active: true });
  };

  const openEditCoupon = (c) => {
    setEditingCoupon(c);
    setCouponForm({ code: c.code, discount_percent: c.discount_percent, max_uses: c.max_uses || 0, valid_plans: c.valid_plans || [], expires_at: c.expires_at || '', active: c.active !== false });
  };
  ...
  const saveCoupon = async () => {
    ...
    try {
      if (editingCoupon?.isNew) {
        await api.post('/admin/coupons', couponForm);
        toast.success('Coupon created');
      } else {
        await api.put(`/admin/coupons/${editingCoupon.code}`, couponForm);
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="coupons" className="space-y-6 mt-4">
          <div className="flex items-center justify-between">
            ...
            <Button onClick={openAddCoupon} data-testid="admin-add-coupon-btn"
              className="rounded-full bg-[#0EA5E9] hover:bg-[#0284C7] text-white px-4 text-sm">
              ...
              <tbody>
                {coupons.map(c => (
                  <tr key={c.code} className="border-b border-[#E2E8F0] hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-5 py-3">
                      <Badge className="bg-[#0EA5E9]/10 text-[#0EA5E9] border-0 text-xs font-semibold">{c.discount_percent}% off</Badge>
                    </td>
                    <td className="px-5 py-3 text-[#64748B] hidden sm:table-cell">
                      {c.used_count || 0}{c.max_uses > 0 ? ` / ${c.max_uses}` : ' (unlimited)'}
                    </td>
                    <td className="px-5 py-3 text-[#64748B] text-xs hidden sm:table-cell">
                      {c.expires_at ? new Date(c.expires_at).toLocaleDateString() : 'Never'}
                    </td>
                    <td className="px-5 py-3">
                      <Badge className={`border-0 text-xs ${c.active !== false ? 'bg-[#10B981]/10 text-[#10B981]' : 'bg-[#EF4444]/10 text-[#EF4444]'}`}>
                    ...
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8" data-testid={`delete-coupon-${c.code}`}
                ...
                {coupons.length === 0 && (
                  <tr><td colSpan={6} className="px-5 py-8 text-center text-[#64748B]">No coupons created yet. Click "Create Coupon" to add one.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          <Dialog open={!!editingCoupon} onOpenChange={(open) => { if (!open) setEditingCoupon(null); }}>
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle style={{ fontFamily: 'Outfit' }}>
                  {editingCoupon?.isNew ? 'Create Coupon' : `Edit: ${editingCoupon?.code || ''}`}
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                <div>
                  ...
                  <Input value={couponForm.code} onChange={e => setCouponForm(f => ({ ...f, code: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '') }))}
                    placeholder="e.g. SAVE20" disabled={editingCoupon && !editingCoupon.isNew} data-testid="coupon-code-form-input"
                    className="mt-1 rounded-xl border-[#E2E8F0] font-mono uppercase" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Input type="number" min={0} value={couponForm.max_uses}
                      onChange={e => setCouponForm(f => ({ ...f, max_uses: parseInt(e.target.value) || 0 }))}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  <Button onClick={saveCoupon} disabled={couponSaving} data-testid="save-coupon-btn"
                    className="rounded-full bg-[#0EA5E9] hover:bg-[#0284C7] text-white px-6">
