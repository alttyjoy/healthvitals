    ...
    <div className="flex flex-col h-full">
        ...
        {isAdmin && (
          <>
            <div className="pt-4 pb-2 px-4">
              <span className="text-xs tracking-[0.15em] uppercase text-[#0EA5E9] font-semibold">Admin</span>
            </div>
            {adminItems.map(item => (
              <NavLink
                key={item.to}
      ...
      <div className="p-4 border-t border-[#E2E8F0]">
        <div className="flex items-center gap-3 px-4 py-2 mb-2">
          <div className="w-8 h-8 rounded-full bg-[#0EA5E9] flex items-center justify-center text-white text-xs font-medium">
            {user?.name?.[0]?.toUpperCase() || 'U'}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium text-[#0F172A] truncate">{user?.name}</div>
            <div className="text-xs text-[#64748B] truncate">{user?.plan?.toUpperCase()} Plan</div>
          </div>
        </div>
        <button
          onClick={onLogout}
          data-testid="sidebar-logout-btn"
          className="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium text-[#64748B] hover:bg-red-50 hover:text-red-600 transition-all w-full"
        >
...
export default function DashboardLayout() {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC]">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 flex-shrink-0 border-r border-[#E2E8F0] bg-white flex-col">
